...
class Friend(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey(
        'user.id'))  # Foreign key column linking User tbale, id of user who sent the request
    ...
    friend = db.relationship('User',
                             backref='friend')  # establish relationship to the user, specifically person who received request
...
with app.app_context():
    ...
    if Location.query.filter_by(latitude=43.982465, longitude=-89.078786, reviewer_count=5).first() == None:
        address = "Seneca, WI"
        rating = 4.2 * 5
        test_pop_1 = Location(name="test_pop_1", reviewer_count=5, rating=rating, latitude=43.982465,
                              longitude=-89.078786, address=address)
    ...
    if Location.query.filter_by(latitude=38.72708333, longitude=-106.47411111, reviewer_count=5).first() == None:
        ...
        test_pop_2 = Location(name="test_pop_2", reviewer_count=5, rating=rating, latitude=38.72708333,
                              longitude=-106.47411111, address=address)
    ...
    if Location.query.filter_by(latitude=46.157574, longitude=-93.166120, reviewer_count=5).first() == None:
        ...
        test_pop_3 = Location(name="test_pop_3", reviewer_count=5, rating=rating, latitude=46.157574,
                              longitude=-93.166120, address=address)
    ...
    if Location.query.filter_by(latitude=45.313161, longitude=-89.315375, reviewer_count=5).first() == None:
        ...
        test_pop_3 = Location(name="test_pop_4", reviewer_count=5, rating=rating, latitude=45.313161,
                              longitude=-89.315375, address=address)
        db.session.add(test_pop_3)
        db.session.commit()
...
async def process_loc(loc, single):
    ...
    if loc_score.score >= 3 or single == True:  # dont forget to change back
        ...
        optimal_loc = (
        {'lat': loc['lat'], 'lng': loc['lng'], 'label': loc['label'], 'ranking': loc_score.return_current_score_str(),
         'ranking_score': loc_score.score,
         'light_ranking': loc_score.return_current_light_pollution_str(), 'weather_report': weather_rep,
         'lunar_phase': lunar_phase,
         'lunar_impact': loc_score.moon_light_pollution_card[loc_score.moon_light_pollution], 'address': address})
        return optimal_loc
    return None
    ...
    if request.method == "POST":
        ...
        if values != None:
            ...
            return redirect(url_for("results", rating=ovrl_ranking, light_rating=light_ranking, lunar_phase=l_phase,
                                    lunar_impact=l_score, address=address))
        ...
        if lat == '' or lng == '':
            ...
            return render_template("find_stars.html", form=form, map_api_key=api_key, map_id=map_id,
                                   usr_coords=zoom_coords, markers=[])
        ...
        return render_template("find_stars.html", form=form, map_api_key=api_key, map_id=map_id, usr_coords=zoom_coords,
                               markers=optimal_locs, popular_markers=popular_markers)
...
def results(rating, light_rating, lunar_phase, lunar_impact, address):
        ...
        for loc in locations:
            ...
            if str(loc_lat) == lat and str(loc_lng) == lng:
                ...
                return render_template("results.html", rating=rating, light_rating=light_rating,
                                       weather_report=weather_report, lunar_phase=lunar_phase,
    ...
    return render_template("results.html", rating=rating, light_rating=light_rating,
                           weather_report=weather_report, lunar_phase=lunar_phase, point=point,
...
def find_constellations(latitude, longitude):
    ...
    for constellation in constellations:
        ...
        if constellations[constellation] >= 7:
            ...
            if db_constellation:
                display_constellations.append({"name": db_constellation.name, "img": db_constellation.img,
...
def calculate_results(latitude, longitude):
    ...
    return redirect(url_for("results", rating=ranking, light_rating=light_ranking, lunar_phase=lunar_phase,
                            lunar_impact=lunar_impact, address=address))
...
def find_constellations_lm():
    ...
    for constellation in constellations:
        db_constellation = Constellation.query.filter(
            func.lower(Constellation.name).ilike(func.lower(constellation))).first()
        if db_constellation:
...
def reviews(address, latitude, longitude):
    ...
    if location:
        ...
        for review in reviews:
            ...
            user_reviews.append(
                {"user": user.username, "date": review.date.date(), "comment": review.comment, "rating": review.rating})
        loc_rating = location.rating / location.reviewer_count
        loc_rating = round(loc_rating, 2)

    return render_template('reviews.html', address=address, lat=latitude, lng=longitude, location=location,
                           reviews=user_reviews, loc_rating=loc_rating)
